...
#
# ★2026-08-04 재조정 — 위 배분은 **비중**만 보고 잡아서 목표를 못 이뤘다.
#   구간 폭이 2.5 / 5 / 7 / 10 / 15 / 20 / 20 도로 제각각이라, 밀도(비중/폭)는
#
#       0.5-3   3-8    8-15   15-25  25-40  40-60  60-80
#       2.000   2.000  2.143  2.400  1.467  0.750  0.450
#
#   가 되어 **0.5~25도가 넓게 평평한 고원**이었다.  비중 최대는 15~25도인데 KDE 는
#   밀도를 그리므로 그림에서는 노란 띠가 계속 아래(저앙각)에 깔렸다.
#   실측 300장도 목표를 거의 그대로 재현했다(채택 elev_target 이 FRAC 과 ±3%p 이내) —
#   **새는 게 아니라 처방 자체가 저앙각 고원이었다.**
#
#   이번에는 밀도가 중간에서 봉우리가 되도록 정규분포를 구간에 적분해 비중을 얻는다.
#   ★사용자 선택: "중간으로 옮기되 **분산을 크게** 해서 한쪽에 몰리지 않게,
#     80도 쪽도 보이게" -> N(30, 38).  좁은 N(28,22) 는 60~80도가 7% 라 그림에서
#     위쪽이 거의 검게 남았다.
#
#       구간     0.5-3  3-8   8-15  15-25  25-40  40-60  60-80
#       비중      2.9    6.2   9.5   14.7   22.7   26.4   17.6
#       밀도     1.160  1.240 1.357 1.470  1.513  1.320  0.880
#
#   밀도 봉우리는 25~40도이고 양옆으로 완만하게 흐른다(최대/최소 비 1.7 — 이전
#   처방의 5.3 에서 크게 평탄해졌다).
#   ★저앙각을 없애지 않는다 — 15도 미만이 18.6% 로 남는다.  사용자가 "0도에서 앞면만
#   보이는 상황도 추론할 수 있어야 한다"고 명시했으므로 꼬리는 반드시 유지한다.
#   (이전에 내가 "앙각 40도 이상으로 밀자"고 제안했다가 바로 이 이유로 철회했다.)
#
# ★주의: `ELEV_V_JOINT` 는 import 시점에 이 값으로 한 번 만들어진다.  실행 중에
#   ELEV_BIN_FRAC 만 바꾸면 반영되지 않으므로 `build_elev_v_joint()` 를 다시 불러야
#   한다 (실측 실험에서 이걸 빠뜨려 before/after 가 같게 나온 적이 있다).
# 이전 값: [0.05, 0.10, 0.15, 0.24, 0.22, 0.15, 0.09]  (밀도 봉우리가 15~25도)
#          [0.025, 0.061, 0.108, 0.190, 0.295, 0.248, 0.073]  N(28,22), 60~80도 7%
ELEV_BIN_FRAC = [0.029, 0.062, 0.095, 0.147, 0.227, 0.264, 0.176]  # 잠정, sums to 1.00
