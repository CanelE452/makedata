...
# scene_1/2/3 only (Pallet_0 = scene.usd excluded), each ~1/3.
...
import v2_pipeline  # ADDITIVE (v2 order-4 pure pipeline: sample_frame/solve_placement)
import randomizers
...
# scene_preset drives the distractor domain filter (distractor_pool_v2) and the
# scene_preset LABEL. indoor/outdoor classification is the Illumination-DR Layer
# (future); until then every frame uses "random-mix" = the full 209 pool. The old
# Sketchfab_model x3 are not in that pool, so they can never be selected.
...
def compute_annotation_v4(pallet_name, pallet_obj, cam_pos, look_at):
    ...
    data = {
        ...
        "objects": [{
            ...
            # The rest are Layer placeholders (null, owning Layer noted).
            # occlusion_fraction = per-corner[9] CONTINUOUS occlusion in [0,1]; there
            # is currently NO per-corner binary field -- closest existing signals are
            # objects[0].raycast_visibility (aggregate over 10 silhouette pts) and
            # efront_kp12.kp12_visible (per-12kp binary, currently null: raycast_fn=None).
            # front_cos / facing_margin_deg ALREADY EXIST (see _existing_refs).
            "v2_labels": {
...
def parse_args():
    ...
    # --- v2 order-4 pipeline entry (ADDITIVE; default OFF = legacy path intact) ---
    # --v2_plan writes N FrameSpecs (Layer-1 sample_frame, pure, no Blender render)
    # to <out_dir>/v2_plan.jsonl and RETURNS before any render. The place/label wiring
    # (solve_placement/realize) is B2/B3 stubs, so this only emits the plan for now.
    p.add_argument("--v2_plan", type=int, default=0,
                   help="v2 dry entry: emit N sample_frame FrameSpecs to v2_plan.jsonl "
                        "and exit (0 = legacy path, the default).")
...
def main():
    ...
    # --- v2 order-4 entry (ADDITIVE, default OFF): emit Layer-1 FrameSpecs, no render.
    # Returns before touching the Blender scene, so the legacy path below is unaffected.
    if args.v2_plan > 0:
        os.makedirs(out_root, exist_ok=True)
        plan_path = os.path.join(out_root, "v2_plan.jsonl")
        _assets = v2_pipeline.load_assets()
        _specs, _ = v2_pipeline.generate_specs(args.v2_plan, args.seed, _assets)
        with open(plan_path, "w", encoding="utf-8") as _f:
            for _s in _specs:
                _f.write(json.dumps(_s.to_dict()) + "\n")
        print(f"[v2_plan] wrote {len(_specs)} FrameSpecs -> {plan_path} "
              f"(sample_frame only; solve_placement/realize = B2/B3 stubs)")
        return
    ...
    for d in (img_dir, json_dir, ov_dir, stats_dir):
