...
- `data/pallet` 레이아웃 정리는 **Stage 2-A~2-D2 로 종료**(2026-07-31). top-level = `assets/ reference/ runs/ manifests/ release/ archive/ README.md _DISTRIBUTION_EXCLUDE.txt` + `isaac_assets/`(NVIDIA EULA 격리, 이동 계획 없음). `archive/` 는 semantic container 만 남았다: `legacy_datasets/{redistributable,noai_baked,partial}` · `packages/{dataset_bundles,background_sources,corrupt}` · `superseded_runs/`(D2 가 199건 수용) · `legacy_scenes/{snapshots,blender_backups}` · 빈 뼈대 `legacy_assets/ nonredistributable/ unidentified/ corrupt/` + `_noai_quarantine_usd/`(라이선스 근거라 이동 금지). UNKNOWN·BLOCKED·planned residual·top-level ZIP·broken ref·release leak 전부 0. 상세는 `_docs/data_pallet_layout.md` §11
